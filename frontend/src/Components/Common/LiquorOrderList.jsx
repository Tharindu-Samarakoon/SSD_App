...
class LiquorOrderList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      orderList: [],
  ...
  async componentDidMount() {
    ...
    await axios
      .get(
        `${APIURL}/liquor_order/get_all_liquor_order_details/${this.state.user_id}`
      )
      .then((response) => {
        this.setState({ orderList: response.data.AllOrderDetails });
        console.log("orderList =>", this.state.orderList);
        console.log("user_id =>", this.state.orderList);
      });
        ...
        <div className="row">
          ...
          <div className="col-10">
            <h3
              ...
              style={{ marginTop: 30, marginLeft: -190 }}
            >
                    ...
                    {this.state.orderList.length > 0 &&
                      this.state.orderList.map((item, index) => (
                        <tr>
                          ...
                          >
                            <p className="text-current">{item.liquor_name}</p>
                          </td>
                          <td
                            data-label="Lead Score"
                            style={{ fontSize: 12, color: "black" }}
                          >
                            <p className="text-current">{item.liquor_type}</p>
                          </td>
                          <td
                            data-label="Lead Score"
                            style={{ fontSize: 12, color: "black" }}
                          >
                            <p className="text-current">{item.liquor_qntity}</p>
                          </td>
...
export default LiquorOrderList;
