...
class LunchFoodMenu extends Component {
  constructor(props) {
    super(props);
    this.state = {
      customer: [],
  ...
  async componentDidMount() {
    ...
    await axios
      .get(`${APIURL}/menulist/get-all-lunch-menu-items`)
      .then((response) => {
        ...
        <section className="portfolio py-5">
          <div className="container py-xl-5 py-lg-3">
            ...
            >
              ...
              <a
                href="/food-menu"
                style={{ margin: 20, color: "black", fontSize: 20 }}
              >
                All Foods Menu
              </a>

              <a
                href="/breakfirst-food-menu"
                style={{ margin: 20, color: "black", fontSize: 20 }}
              >
                Breakfirst Foods Menu
              </a>

              <a
                href="/lunch-food-menu"
                style={{ margin: 20, color: "blue", fontSize: 20 }}
              >
                Lunch Foods Menu
              </a>
                ...
                this.state.menuList.map((item, index) => (
                  <div className="col-md-4" key={index}>
                    <div className="gallery-demo">
                      <img src={item.item_url} alt=" " className="img-fluid" />
                      <h4 className="p-mask">{item.item_name}</h4>
                      {item.item_price_type_small !== 0 &&
                        item.item_price_type_medium !== 0 &&
                        item.item_price_type_large !== 0 && (
                          <h4 className="p-mask">
                            Small Price (Rs.) :
                            <span>{item.item_price_type_small}</span>
                          </h4>
                        )}
                      ...
                      {item.item_price_type_medium !== 0 && (
                        <h4 className="p-mask">
                          Medium Price (Rs.) :
                          <span>{item.item_price_type_medium}</span>
                        </h4>
                      )}
                      ...
                      {item.item_price_type_large !== 0 && (
                        <h4 className="p-mask">
                          Large Price (Rs.) :
                          <span>{item.item_price_type_large}</span>
                        </h4>
                      )}

                      <Link
                        to={`/single-food-item/${item._id}`}
                      ...
                      <Link
                        to={`/cart-food-item/${item._id}`}
                        className="button-w3ls  mt-2 mb-2"
                        style={{
                          marginLeft: "25%",
                          width: "50%",
                          textAlign: "center",
                      ...
                      >
                        Add to Cart
                        <span className="fa fa-plus ml-1" aria-hidden="true" />
                      </Link>
                    </div>
                  </div>
                ))}
