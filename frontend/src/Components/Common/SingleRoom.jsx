...
class SingleRoom extends Component {
  ...
  async componentDidMount() {
    ...
    await axios
      ...
      .then((response) => {
        this.setState({ singleItem: response.data.RoomDetails });
        console.log("singleItem =>", this.state.singleItem);

        this.setState({
          room_id: this.state.singleItem.room_id,
        });

        this.setState({
          room_name: this.state.singleItem.room_name,
        });

        this.setState({
          bed_type: this.state.singleItem.bed_type,
        });

        this.setState({
          room_price: this.state.singleItem.room_price,
        });
        this.setState({
          room_url: this.state.singleItem.room_url,
        });
      });
  }
  ...
  toggle() {
    this.setState({
      total_price:
        this.state.room_price * this.state.no_of_room * this.state.person_count,
    });
  }
  ...
  onSubmit(event) {
    ...
    let OrderDetails = {
      ...
      no_of_room: this.state.no_of_room,
      room_url: this.state.room_url,
      total_price: this.state.total_price,
      user_id: this.state.user_id,
      user_name: this.state.user_name,
    };

    console.log("room_url", this.state.room_url);
    console.log("OrderDetails", OrderDetails);
    axios
      .post(`${APIURL}/room_order/room_order_place`, OrderDetails)
      .then((res) => {
        console.log("res", res);

        if (res.data.code === 200) {
          toast.success(res.data.message);
          this.props.history.push("/room_list_customer");
        } else {
              ...
              <div className="container py-xl-5 py-lg-3">
                <div className="title-section text-left mb-md-5 mb-4">
                  ...
                  <h3 className="w3ls-title">
                    {this.state.singleItem.room_name} -
                    <span> {this.state.singleItem.room_type}</span>
                  </h3>
                </div>
                      ...
                      <div className="blog-grid-top">
                        <div className="b-grid-top">
                          <div className="blog_info_left_grid">
                            <a href="single.html">
                              <img
                                src={this.state.singleItem.room_url}
                                className="img-fluid"
                                alt=""
                              />
                            </a>
                        ...
                        <h3 className="single-text text-da font-weight-light mt-3">
                          Price (Rs.) - {this.state.singleItem.room_price}
                        </h3>
                      </div>
                    </div>
                  </div>
