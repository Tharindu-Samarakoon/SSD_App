...
class RoomList extends Component {
  ...
  async componentDidMount() {
    ...
    await axios
      .get(`${APIURL}/rooms/get-all-rooms-details`)
      .then((response) => {
        this.setState({ roomsList: response.data.RoomsDetails });
                ...
                this.state.roomsList.map((item, index) => (
                  <div className="col-md-4" key={index}>
                    <div className="gallery-demo">
                      <img src={item.room_url} alt=" " className="img-fluid" />
                      <h4 className="p-mask">{item.room_id}</h4>
                      <h4 className="p-mask">{item.room_name}</h4>
                      <h4 className="p-mask">
                        Bed Type :<span>{item.bed_type}</span>
                      </h4>
                      <h4 className="p-mask">
                        Price (Rs.) :<span>{item.room_price}</span>
                      </h4>
                      <Link
                        to={`/get_room_details/${item._id}`}
