...
const GetLaundryScheduleReports = () => {
    ...
    async function gedData() {
      try {
        const response = await axios.get(
          `${APIURL}/laundry/get-all-schedule-details`
        );
        if (response.status === 200) {
          SetOrder(response.data.LaundryScheduleDetails);
          setBaseData(response.data.LaundryScheduleDetails);
        }
      } catch (error) {
  ...
  const downloadReport = () => {
    doc.text("Laundry Schedules Report", 30, 10);

    let array = [];
    order.map((orders, index) => {
      let row = [];
      row.push(index + 1);
      row.push(orders.laundry_schedule_id);
      row.push(orders.schedule_name);
      row.push(orders.schedule_type);
      row.push(orders.schedule_weight);
      row.push(orders.schedule_price);
      row.push(orders.total_schedule_price);
      row.push(orders.deliver_date);
      array.push(row);
      return row;
    });
  ...
  const search = (inp) => {
    if (!inp.target.value) {
      ...
      let searchList = baseData.filter(
        (data) =>
          data.laundry_schedule_id
            .toLowerCase()
            .includes(inp.target.value.toLowerCase()) ||
          data.schedule_name
            .toLowerCase()
            .includes(inp.target.value.toLowerCase()) ||
          data.schedule_type.toLowerCase().includes(inp.target.value.toLowerCase())
      );
      SetOrder(searchList);
    }
  };
...
export default GetLaundryScheduleReports;
