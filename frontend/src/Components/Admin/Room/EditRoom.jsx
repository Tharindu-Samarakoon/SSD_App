...
class EditRoom extends Component {
  ...
  async componentDidMount() {
    await axios
      .get(
        `${APIURL}/rooms/get-room-details-by-id/${this.props.match.params.id}`
      )
      .then((response) => {
        this.setState({ roomDetails: response.data.RoomDetails });
        console.log("roomDetails =>", this.state.roomDetails);

        this.setState({ room_id: this.state.roomDetails.room_id });
        this.setState({ room_name: this.state.roomDetails.room_name });
        this.setState({ room_type: this.state.roomDetails.room_type });
        this.setState({ bed_type: this.state.roomDetails.bed_type });
        this.setState({ person_count: this.state.roomDetails.person_count });
        this.setState({ room_meals: this.state.roomDetails.room_meals });
        this.setState({ room_price: this.state.roomDetails.room_price });
        this.setState({ image: this.state.roomDetails.room_url });
      });
  }
  ...
  onSubmit(event) {
    ...
    axios
      .put(
        `${APIURL}/rooms/update-room-details/${this.props.match.params.id}`,
        formData
      )
      .then((res) => {
        console.log("res", res);
        ...
        if (res.data.code === 200) {
          ...
          this.props.history.push("/get-all-rooms-details");
        } else {
  ...
  onDelete(event) {
    ...
    axios
      .delete(`${APIURL}/rooms/delete-room/${this.props.match.params.id}`)
      .then((res) => {
        console.log("res", res);
        if (res.data.code === 200) {
          toast.success(res.data.message);
                              ...
                              <div className="p-5">
                                <div className="text-center">
                                  <h1 className="h4 text-gray-900 mb-4">
                                    Edit {this.state.roomDetails.room_name}'s
                                    Details
                                  </h1>
                                </div>
                                <form
