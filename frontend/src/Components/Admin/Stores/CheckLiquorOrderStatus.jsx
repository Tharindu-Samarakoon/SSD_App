...
const CheckLiquorOrderStatus = () => {
    ...
    async function gedData() {
      try {
        const response = await axios.get(
          `${APIURL}/liquor_order/get_all_order_details`
        );
  ...
  const downloadReport = () => {
    doc.text("Liquor Orders Status Report", 30, 10);
    ...
    order.map((orders, index) => {
      ...
      row.push(orders._id);
      row.push(orders.liquor_name);
      row.push(orders.liquor_type);
      row.push(orders.liquor_qntity);
...
export default CheckLiquorOrderStatus;
